...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.redirect(
      `${(process.env.NEXTAUTH_URL ?? process.env.NEXT_PUBLIC_APP_URL ?? "http://localhost:3000").replace(/\/$/, "")}/dashboard/buyer/settings?connected=procore`
    );
  } catch (err) {
    console.error("[procore/callback]", err);
    return NextResponse.redirect(
      `${(process.env.NEXTAUTH_URL ?? process.env.NEXT_PUBLIC_APP_URL ?? "http://localhost:3000").replace(/\/$/, "")}/dashboard/buyer/settings?error=procore_failed`
    );
  }
}
