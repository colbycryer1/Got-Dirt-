...
import DailyHaulRateLock from "./DailyHaulRateLock";
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-6 py-10">
        <div className="flex items-start justify-between mb-8">
          <h1 className="text-2xl font-bold text-gray-900">My Pits</h1>
          <p className="text-xs text-gray-500 max-w-xs text-right">
            Lock today&apos;s haul rate on each pit to broadcast jobs to all verified drivers <em>and</em> carriers.
          </p>
        </div>
          ...
          <div className="space-y-4">
            {pits.map((pit) => (
              <div key={pit.id} className="bg-white rounded-2xl border border-gray-200 p-5">
                ...
                {/* Daily haul rate lock — required before broadcasting to carriers */}
                <DailyHaulRateLock
                  pitId={pit.id}
                  currentRateCents={pit.dailyHaulRateCents ?? null}
                  lockedAt={pit.dailyHaulRateLockedAt?.toISOString() ?? null}
                />
              </div>
            ))}
          </div>
