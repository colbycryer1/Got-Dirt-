  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/projects", {
        ...
        body:    JSON.stringify({ name: name.trim(), location: location.trim() || undefined, description: description.trim() || undefined }),
      });
      if (!res.ok) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-lg mx-auto px-6 py-10">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-200 p-6 space-y-5">
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1">Job Site Location</label>
            <input
              className="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500"
